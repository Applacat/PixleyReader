...
final class RecentFoldersManagerTests: XCTestCase {
    ...
    func testAddFolder_addsToList() {
        ...
        manager.addFolder(url, bookmarkData: "bookmark".data(using: .utf8)!)
    ...
    func testAddFolder_removesDuplicatePath() {
        ...
        let url = URL(fileURLWithPath: "/Users/test/Documents")
        manager.addFolder(url, bookmarkData: "old_bookmark".data(using: .utf8)!)

        // When: Add same path again
        manager.addFolder(url, bookmarkData: "new_bookmark".data(using: .utf8)!)
    ...
    @MainActor
    func testRefreshStaleBookmark_preservesOrder() {
        ...
        manager.addFolder(url1, bookmarkData: "b1".data(using: .utf8)!)
        // Small delay to ensure different timestamps
        Thread.sleep(forTimeInterval: 0.01)
        manager.addFolder(url2, bookmarkData: "b2".data(using: .utf8)!)
        Thread.sleep(forTimeInterval: 0.01)
        manager.addFolder(url3, bookmarkData: "b3".data(using: .utf8)!)
        ...
        let folder1 = folders.last!  // url1 is last (oldest)
        manager.refreshStaleBookmark(folder1, url: url1, newBookmarkData: "new_b1".data(using: .utf8)!)
    ...
    @MainActor
    func testRefreshStaleBookmark_doesNotCreateDuplicate() {
        // Given: Single folder
        let url = URL(fileURLWithPath: "/Users/test/Documents")
        manager.addFolder(url, bookmarkData: "old".data(using: .utf8)!)
        XCTAssertEqual(manager.getRecentFolders().count, 1)

        let folder = manager.getRecentFolders().first!

        // When: Refresh
        manager.refreshStaleBookmark(folder, url: url, newBookmarkData: "new".data(using: .utf8)!)
    ...
    @MainActor
    func testOldRefreshBehavior_changesOrder() {
        ...
        let folder1 = folders.last!  // url1 is last
        manager.refreshStaleBookmarkOldWay(folder1, url: url1, newBookmarkData: "new_b1".data(using: .utf8)!)
    ...
    func testRefreshStaleBookmark_nonexistentFolder_noOp() {
        ...
        let ghostFolder = TestableRecentFolder(
            ...
            bookmarkData: "ghost".data(using: .utf8)!
        )

        // When: Try to refresh
        manager.refreshStaleBookmark(ghostFolder, url: URL(fileURLWithPath: "/nonexistent"), newBookmarkData: "new".data(using: .utf8)!)
    ...
    @MainActor
    func testRefreshStaleBookmark_preservesName() {
